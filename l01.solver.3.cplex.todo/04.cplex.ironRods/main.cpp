...
void setupLP(CEnv env, Prob lp)
{
	...
	// add z var [in o.f.: ... + (L-F) z ]
	{
		...
		const double D = L-F;
		char* xname = "z";
		CHECKED_CPX_CALL( CPXnewcols, env, lp, 1   ,&D, &lb, &ub, &xtype, &xname );
	...
	for (int i=0; i<I; i++){ //disponibilita'
		std::vector<int> rmatbeg;
		std::vector<double> coef(J,1.0);

        rmatind.push_back(i*J+0);
        rmatind.push_back(i*J+1);
        rmatind.push_back(i*J+2);
